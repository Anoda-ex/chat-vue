        ...
        <div class="overlay">
            <div class="popup__wrapper" v-click-outside="()=>{$emit('close')}">
                <div class="popup__header">
                    <div class="poput-title">{{`Invite users to "${chatData.name}".`}}</div>
                ...
                <div class="popup__body">
                    <ContactsList 
                        :items="$store.getters.contactsDataForChatFiltered(chatData.members)" 
                        type="checkboxes" 
                        @selectedUsersChange="selectedUsers=$event"
                    ></ContactsList>
                    <div class="flex-center mt-30">
                        <button @click="$emit('confirm', selectedUsers)" class="btn btn--blue mt-40">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
